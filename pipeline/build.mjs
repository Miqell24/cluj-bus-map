...
// Cluj-Napoca: ONE CTP Cluj feed split by route_type — buses and trolleybuses (route_type 3 and 11) on the
// road graph, trams (0) on the tram graph. Trolleybuses ride the bus network in green
...
// swallow the tram lines too.
// Cluj-Napoca has no metro, so the third cfg its siblings carry is absent here.
const MODES = [{
  mode: 'bus', label: 'buses', osmFile: 'data/osm/cluj.json',
  graphMode: 'road', color: '#0059a9', colorDark: '#00294f',
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['24']),
  feeds: [
    {
...
if (tramAll || tramSel.length) MODES.push({
  mode: 'tram', label: 'trams', osmFile: 'data/osm/cluj-rail.json',
...
async function processMode(cfg) {
  ...
  for (const feed of cfg.feeds) {
    ...
    for (const s of await readCsv(join(fdir, 'stops.txt'))) {
      ...
      // CTP Cluj publishes its stop names properly cased and accented, so
